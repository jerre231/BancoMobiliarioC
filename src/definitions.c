...
int BuyAndSellHousesMenu(struct player *currentPlayer, struct house houses[40])   // Lembrar de sempre chamar a funcao printPlayerInfo() depois de executar essa funcao, pois ela vai apagar oq foi printado antes
{
    ...
    printf("%s ",currentPlayer->nome);
    changeTextColour(RESET);
    printf("DINHEIRO EM MAOS: $%d VALOR EM BENS: $%d");
    move(95,5); printf("BENS DO JOGADOR: ");
    int startlineLIST = 6;
    for(int i = 0; i < currentPlayer->numberofProperties; i++)
    {
        move(95,startlineLIST+i); printf("%d- %s (%d casas e %d hotels, aluguel atual: $%d)", propertiesPlayer[i], houses[propertiesPlayer[i]].name, houses[propertiesPlayer[i]].housesBuilt, houses[propertiesPlayer[i]].hotelBuilt, houses[propertiesPlayer[i]].rent);
    }
    ...
    choicebuyorsell: move(95,29); char opcao = getc("");
    switch (opcao) {
        case '1':
            move(95,30); printf("Escolha qual propriedade voce deseja administrar.");
            choicewheretobuy: move(95,31); int opcaoProp = (int)getc("");
            ...
            choicewannabuyhouse: move(95, 37); char opcao2 = getc("");
            switch (opcao2) {
                case '1':
